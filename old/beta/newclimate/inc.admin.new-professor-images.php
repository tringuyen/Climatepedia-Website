...
//If the error occures the file will not be uploaded.
 ...
 if (($extension != "jpg") && ($extension != "jpeg") && ($extension != "png") && ($extension != "gif")) 
		...
		//print error message
 			...
 			$errors=1;
 			header("Location:http://www.climatepedia.org/beta/newclimate/admin.php?console=podium&action=failure&type=image&error_message=" . $error_message);
 		}
 		else
 		{
...
$newname="images_podium/professors/".$image_name;
...
$query = ("INSERT INTO podium_images (image_type, image_parent, image_path, image_description, image_source) VALUES ('1', '$post_id', '$image_path', '$image_description', '$image_source')");
...
$i++;
 	...
 	$post_id = $_POST['new_submit_images_post'];
 	include ("inc.admin-user.connect.php");
 	$query = ("SELECT professor_id, professor_title, professor_firstname, professor_lastname FROM podium_professors WHERE professor_id='$post_id'");
	$result=$db->query($query);
	...
	header("Location:http://www.climatepedia.org/beta/newclimate/admin.php?console=podium&action=success&type=new&query=professor_image&title=" . $post_name);
