...
$newname="images_podium/universities/".$image_name;
...
$query = ("INSERT INTO podium_images (image_type, image_parent, image_path, image_description, image_source) VALUES ('2', '$post_id', '$image_path', '$image_description', '$image_source')");
...
$i++;
 	...
 	$post_id = $_POST['new_submit_images_post'];
 	include ("inc.admin-user.connect.php");
 	$query = ("SELECT university_id, university_name FROM podium_universities WHERE university_id='$post_id'");
	$result=$db->query($query);
	...
	header("Location:http://www.climatepedia.org/beta/newclimate/admin.php?console=podium&action=success&type=new&query=university_image&title=" . $post_name);
