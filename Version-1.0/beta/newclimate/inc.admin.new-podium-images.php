...
$newname="images_podium/posts/".$image_name;
...
$query = ("INSERT INTO podium_images (image_type, image_parent, image_path, image_description, image_source) VALUES ('3', '$post_id', '$image_path', '$image_description', '$image_source')");
...
$i++;
 	...
 	$post_id = $_POST['new_submit_images_post'];
 	include ("inc.admin-user.connect.php");
 	$query = ("SELECT post_id, post_name, post_url FROM podium_posts WHERE post_id='$post_id'");
	$result=$db->query($query);
	...
	header("Location:http://www.climatepedia.org/beta/newclimate/admin.php?console=podium&action=success&type=new&query=post_image&title=" . $post_name . "&url=" . $post_url);
