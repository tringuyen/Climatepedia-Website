...
<div id="wrapper"> <!-- Wraps whole page content -->
	...
	<div id="focus_bar">
		...
		<div id="focus_sort">
			<a href="#" id="feed_sort_button" >Sort News by<img src="/beta/newclimate/css/images/arrow_down.png" alt="" ></a>
			<div id="feed_sort_wrapper">
				<div id="feed_sort_list" class="shadow">
					<ul>
						<li><a href="/beta/newclimate/news/highlights" >Highlights</a></li>					
						<li><a href="/beta/newclimate/news/<?php echo $_GET['keyword'] . '/page/' . $page_number; ?>&sort=newest" >Newest</a></li>		
						<li><a href="/beta/newclimate/news/<?php echo $_GET['keyword'] . '/page/' . $page_number; ?>&sort=oldest" >Oldest</a></li>
						<li><a href="/beta/newclimate/news/<?php echo $_GET['keyword'] . '/page/' . $page_number; ?>&sort=alphabetical" >A - Z</a></li>
						<li><a href="/beta/newclimate/news/<?php echo $_GET['keyword'] . '/page/' . $page_number; ?>&sort=inverse" >Z - A</a></li>								
					</ul>
				</div>
			</div>				
		</div>
				...
				<ul>
					<?php // pull system-wide tags that are enabled for the news feed
							...
							while($row=$result->fetch_array()) {
								...
								echo ("<li><a href=/beta/newclimate/news/" . $list_keyword . ">" . $row['tag_name'] . "</a></li>");
							}
					?>
				</ul>
			<p>Special streams:</p>
				...
				<?php
							...
							while($row=$result->fetch_array()) {
								...
								echo ("<li><a href=/beta/newclimate/news/" . $list_source . ">" . $row['source_sidebar'] . "</a></li>");
							}
				?>
				</ul>
			<p>Main streams:</p>
	...
	<div id="right">
		...
		<div id="images_nasa" class="right_section">
			<h3 class="right_header">Recent NASA images</h3>
				<?php include("inc.main.connect.php");
					$query = ("SELECT item_title, item_link, item_date, item_description, item_source FROM news_items WHERE item_source = 'NASA Earth Observatory Natural Hazard Images' ORDER BY item_date DESC LIMIT 3");
					$result=$db->query($query);
					while($row=$result->fetch_array()) {
						$images_description = preg_replace('/<img src=\"http:\/\/feeds[^>]+\>/i', '', $row["item_description"]); 
						echo $images_description;
